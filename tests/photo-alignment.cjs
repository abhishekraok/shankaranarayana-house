...
(async()=>{const browser=await chromium.launch({channel:process.env.BROWSER_CHANNEL||'msedge',headless:true});try{
 ...
 let diskWrites=0;await context.route('**/local-photos/align-poses.json',r=>{if(r.request().method()==='POST'){diskWrites++;r.fulfill({status:204});}else r.fulfill({status:404});});
 ...
 await page.evaluate(k=>{localStorage.removeItem(k);localStorage.removeItem('shankaranarayana.align-position.v1');},KEY);
 ...
 assert.deepEqual(errors,[]);console.log(JSON.stringify({passed:true,checks:['side-by-side aspect match','keyboard height and zoom','space saves and advances','previous restores pose','JSON export','reload resumes','invalid import rejection','close restores view','tour restart','portrait stacking'],errors}));
