...
(async()=>{
 ...
 const bank=await page.evaluate(()=>{
  ...
  return {before,moved,returned,front};
 });
 for(const key of ['before','moved','returned'])assert.ok(Math.abs(bank[key].position[1]-bank[key].feet-2.12)<.01,'Far-bank camera gains half a metre without changing feet');
 assert.ok(Math.abs(bank.front.position[1]-bank.front.feet-1.62)<.01,'House walking height stays unchanged');
 ...
 const routes=await page.evaluate(()=>{
  ...
  return [route('side bay central steps',[32,.1,9.1],[[32,7.1],[32,9.1]]),route('upper hall seating',[41.8,4.08,5.3],[[41.8,2.7],[44.6,2.7],[44.6,.37],[32,.37]]),route('inner sanctum aisles',[39,.1,13.6],[[39,20.2],[35.2,20.2],[35.2,27.8],[42.8,27.8],[42.8,20.2],[39,20.2],[39,13.6]]),route('covered hall',[26.1,.60,14.4],[[26.1,10.0],[25.2,10.0],[25.2,8.2]]),route('front passage',[0,0,-2],[[0,5.1]]),route('entrance to courtyard',[0,0,-2],[[0,4.55],[-2.9,4.55],[-2.9,5.15],[-4.9,5.15],[-4.9,7.0],[-3.7,7.0],[-3.7,11],[-5,10.8]]),route('house upstairs',[-13.3,.45,.95],[[-7.93,.95],[-2.65,.95]]),route('temple court',[39,0,-3.8],[[39,8.8],[33,8.8],[33,14]]),route('kitchen around courtyard stair',[-5,.035,10.8],[[-5,13],[-7.75,13],[-7.75,10.8],[-10.05,10.8],[-10.05,15.5]]),route('courtyard stair up',[-6.75,.035,12.8],[[-6.75,12.35],[-6.75,5.5]]),route('courtyard stair down',[-6.75,3.85,5.5],[[-6.75,12.35],[-6.75,12.8]]),route('lake paired stairs',[15.2,.055,-10.1],[[15.2,-12.65],[19.3,-12.65],[27.5,-12.65],[30.7,-12.65]]),route('lake paired stairs return',[30.7,.065,-12.65],[[27.5,-12.65],[19.3,-12.65],[15.2,-12.65],[15.2,-10.1]]),route('pavilion entry',[28,-.29,-43.3],[[28,-40]])];
 });
