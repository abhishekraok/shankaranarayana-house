...
http.createServer((req,res)=>{let rel;try{rel=decodeURIComponent(new URL(req.url,'http://localhost').pathname);}catch{res.writeHead(400);res.end();return;}
 // Alignment saves are mirrored to DIR/align-poses.json so they survive browser changes.
 if(req.method==='POST'&&rel==='/local-photos/align-poses.json'&&photoDir){
  let body='';req.setEncoding('utf8');
  req.on('data',c=>{body+=c;if(body.length>4e6){res.writeHead(413);res.end();req.destroy();}});
  req.on('end',()=>{try{const data=JSON.parse(body);if(data.schema!=='shankaranarayana-photo-poses'||!Array.isArray(data.poses))throw Error();
   fs.writeFile(path.join(photoDir,'align-poses.json'),JSON.stringify(data,null,1),err=>{res.writeHead(err?500:204);res.end();});}
